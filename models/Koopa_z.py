...
class KPLayer(nn.Module):
    ...
    def one_step_forward(self, z, return_rec=False):
        ...
        # --- 新增：算子谱修正 (Spectral Clipping) ---
        # 强制算子符合物理稳定性偏置
        with torch.no_grad():
            # 计算算子的 Frobenius 范数作为谱半径的近似指标
            # 也可以使用复杂的特征值分解，但 norm 计算速度更快
            k_norm = torch.norm(self.K, p=2, dim=(-2, -1), keepdim=True)
            # 设定一个物理阈值（如0.99），超过该值则强制缩放到0.99产生阻尼效应
            stability_threshold = 0.99
            mask = k_norm > stability_threshold
            self.K = torch.where(mask, self.K * (stability_threshold / k_norm), self.K)
...
class TimeInvKP(nn.Module):
    ...
    def forward(self, x):
        ...
        # --- 新增：融入阻尼/增长算子 ---
        # 创建预测时间轴索引 t = [0, 1, ..., pred_len-1]
        t = torch.arange(self.pred_len).to(res.device).float()
        # 构造演化权重 exp(sigma * t)
        # 我们对 t 进行归一化处理 (t / pred_len) 以保证训练稳定性
        evolution_weight = torch.exp(self.sigma * (t / self.pred_len)) # 维度: [S]
        
        res = self.decoder(res) # B C S
        
        # 将阻尼/增长权重作用于输出。维度对齐：[B, C, S] * [1, 1, S]
        res = res * evolution_weight.view(1, 1, -1)
