...
if [ -z "${WEATHER_AR_SKIP_ORPHAN_CHECK:-}" ] && git rev-parse --git-dir >/dev/null 2>&1; then
  ...
  case "$current" in
    main|master)
      echo "exchange_feature_autoresearch_patchtst: 当前在 '$current'，禁止在此跑 trial。" >&2
      exit 1
      ;;
  esac
  for base in main master; do
    if git show-ref --verify --quiet "refs/heads/$base" 2>/dev/null; then
      if git merge-base HEAD "$base" >/dev/null 2>&1; then
        echo "exchange_feature_autoresearch_patchtst: 当前分支与 '$base' 存在共同祖先，不符合 orphan 实验线。" >&2
        exit 1
      fi
    fi
  done
...
if git rev-parse --git-dir >/dev/null 2>&1; then
  ...
  if git diff --cached --quiet -- dig.py 2>/dev/null; then
    git commit --allow-empty -m "exp(exchange-patchtst): ${DESC}"
  else
    git commit -m "exp(exchange-patchtst): ${DESC}"
  fi
fi
