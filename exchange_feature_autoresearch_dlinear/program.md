# Exchange Feature AutoResearch — DLinear 实验线（TSLib 汇率长期预测特征挖掘）
...
```text
/data/nishome/xuhaochen/tslib-wt/exchange-dlinear
```

仓库根即该 worktree 目录（内含完整 TSLib 与 `dataset/`）。子目录：`exchange_feature_autoresearch_dlinear/`（本文件所在目录）。

与 `exchange_feature_autoresearch/`（iTransformer）、`exchange_feature_autoresearch_patchtst/`（PatchTST）及全部 **Weather** 实验线 **独立**：各自 `baselines.json`、`results_all.tsv`，**互不引用**对方 baseline。`model_id` 使用前缀 **`exchange_`**，与 `weather_*` checkpoint 区分。

当前 worktree 建议分支：**`auto/20260528-exchange-dlinear`**（orphan，与 `main`/`master` 无共同祖先）。
...
3. **GPU**：`prepare.py` 默认 `CUDA_VISIBLE_DEVICES=0`。并行跑多条实验线时，**每条线指定不同卡**：
   ...
   ```bash
   python exchange_feature_autoresearch_dlinear/prepare.py ... --cuda-visible-devices 1
   ```
...
> 下面内容可直接复制给 agent，作为启动 prompt（**Exchange + DLinear** 实验线；先 setup，再开跑 trial）。
...
- 本线 worktree 路径：`/data/nishome/xuhaochen/tslib-wt/exchange-dlinear`
- 分支名：`auto/20260528-exchange-dlinear`；标签格式：`YYYYMMDD-exchange-dlinear`（例：`20260528-exchange-dlinear`）。
...
- `exchange_feature_autoresearch_dlinear/dig.py`（**唯一可改**：`FEATURE_SET_NAME` + `compute_features`）
...
- 日志路径：`exchange_feature_autoresearch_dlinear/logs/baseline_<UTC>.log`。
- 确认 `exchange_feature_autoresearch_dlinear/baselines.json` 在 `"96"` 下含 **`val_mse`**（旧版仅 test `mse` 须重跑 baseline）。

#### 5) 审计文件规则

- `exchange_feature_autoresearch_dlinear/results_all.tsv`：每次 trial 运行都要追加一行，**禁止 commit**。

#### 6) setup 完成后先汇报，等待用户确认

- 必须输出：当前分支名、最近 5 条 commit、`baselines.json` 是否存在及 `96` 的 baseline **val_mse** 摘要。
- 明确询问：是否进入正式实验循环？
- 在用户未明确回复「开始/继续」前：**禁止**修改 `dig.py` 中的特征、**禁止**运行 `bash exchange_feature_autoresearch_dlinear/run_experiment.sh`。

### Phase B（仅在用户确认后执行）

#### 7) 进入实验循环

- 每轮最小单元：`bash exchange_feature_autoresearch_dlinear/run_experiment.sh "本轮说明"`（可选 `--train-epochs` 等，同 `prepare.py`，默认 10；默认仅 horizon 96）。
- 每轮必须留下：独立 commit（若有 git）+ `exchange_feature_autoresearch_dlinear/logs/experiment_<UTC>.log` + `results_all.tsv` 新增一行。
...
- 会话日志：`exchange_feature_autoresearch_dlinear/logs/`（`mkdir -p` 自动创建）。
- `run_experiment.sh` 会拒绝在 `main`/`master` 上运行，并校验与 `main`/`master` **无共同祖先**；非 git 目录会跳过 commit 与该校验，仍会写日志与 `results_all.tsv`。
- 若 `Permission denied`：使用 `bash exchange_feature_autoresearch_dlinear/run_experiment.sh "..."`，或对脚本 `chmod +x`。
...
| 模型 | 默认 **`DLinear`**（与 TSLib `long_term_forecast` / Exchange 脚本一致：`e_layers=2`）；**改模型后须重跑 baseline** |
| `e_layers/d_layers/factor/n_heads` | 2 / 1 / 3 / 8（`n_heads` 仅用于 checkpoint `setting` 命名） |
...
```bash
python exchange_feature_autoresearch_dlinear/append_results_all.py --log ... --min-improve 0.001
```
...
python exchange_feature_autoresearch_dlinear/prepare.py \
  --mode baseline \
  --log-file auto \
...
```bash
cd /data/nishome/xuhaochen/tslib-wt/exchange-dlinear
bash exchange_feature_autoresearch_dlinear/run_experiment.sh "本轮描述" --cuda-visible-devices 0
```

**查看账本**：

```bash
column -t -s $'\t' exchange_feature_autoresearch_dlinear/results_all.tsv | less -S
```
...
grep -E "^(feature_set_name|feature_fp|n_features|staging_csv|baseline_val_mse|val_mse|val_improve|score|test_mse):" \
  exchange_feature_autoresearch_dlinear/logs/experiment_*.log | tail -n 50
```
...
激活 **tslib** → **`cd /data/nishome/xuhaochen/tslib-wt/exchange-dlinear`**（分支 **`auto/20260528-exchange-dlinear`**）→ **DLinear baseline**（`train_epochs=10`）→ **`baselines.json` 含 val_mse@96** → **`results_all.tsv` 勿 commit** → 汇报并等 **「开始/继续」** → **Phase B**：`run_experiment.sh`，只改 **`dig.py`**；**score = 验证集相对本线 baseline @ pl=96**；**`model_id` 前缀 `exchange_`**；**`generated_data/` 默认空**。
